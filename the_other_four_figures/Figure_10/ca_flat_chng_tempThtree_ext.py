...
09Aug19: This script is a followup of ca_flat_chng_temporalThetatree.py
...
th_tr = np.zeros(data['n_years'], dtype=float)
...
current_time = 10001.032110274204  # Start from first day of Jan
...
time_check = 10001.  # Buffer to store current_time at previous storm
yrs = 10001  # Keep track of number of years passed
water_stress = 0.  # Buffer for Water Stress
Tg = 0  # Growing season in days

# Saving
try:
    os.chdir('E:/Research_UW_Sai_PhD/2019_CATGraSS_SpDist_clust_ext')
except OSError:
    os.chdir('/data1/sai_projs/2019_CATGraSS_SpDist_clust_ext')
finally:
    pass
...
# Run storm Loop
i = 666656  # check below how I got to this number (for start of year 10002)
while yrs < n_years:
    i += 1
    # Update objects
    ...
    if (current_time - time_check) >= 1.:
        ...
        grid1.at_cell['vegetation__cumulative_water_stress'] = (
            cum_water_stress / Tg)
        # 01Aug19: Temporally varying Thetatree - using uniform distribution
        th_tr_min = data['ThetaTree'] - data['th_tr_variability']
        th_tr_max = data['ThetaTree'] + data['th_tr_variability']
        th_tr[yrs] = th_tr_min + np.random.rand()*(th_tr_max - th_tr_min)
        vegca._th_tr = th_tr[yrs]
...
## Jai Sri Sainath!
## 09Aug19: How I got to know numbers for
## i, yrs, time_check, and current_time
#if 1:
#    yrs = 0
#    i = -1
#    time_check = 0
...
#    while yrs < 10001:
#        i += 1
#        current_time += (storm_dt[i]+inter_storm_dt[i])/(365. * 24.)
#        if (current_time - time_check) >= 1.:
#            time_check = np.floor(current_time)
#            yrs += 1