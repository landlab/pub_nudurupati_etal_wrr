...
# Saving
try:
    os.chdir('E:/Research_UW_Sai_PhD/2018_CATGraSS_SpDist')
except OSError:
    os.chdir('/data1/sai_projs/2018_CATGraSS_SpDist')
finally:
    pass
    ...
    if (current_time - time_check) >= 1.:
        ...
        if fires:
            if yrs%10 == 0:
                ...
                per_yr_burnt_area_fr[yrs] = np.float(
                        len(burnt_locs))/np.float(grid1.number_of_cells)
        if grazing:
