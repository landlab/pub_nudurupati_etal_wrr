...
data = load_params('input_file.yaml')  # Creates dictionary that holds the inputs
# sub_fldr_name = sys.argv[1] + '_lnger_1'
# data = load_params(sys.argv[1]+'.yaml')  # Creates dictionary that holds the inputs


(grid, elevation) = read_esri_ascii('sev5m_nad27.txt')    # Read the DEM  sev5m_nad27.txt  (285, 799) hugo10mws.txt  OR hugo5m_wgs84.txt (98 148)
...
# this filed should be stored in the folder this program is run
if use_preloaded_pft:
    initial_pft_field = np.load('vegtype_4000.npy')
